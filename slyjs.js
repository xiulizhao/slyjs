(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.slyjs) {
      window.wwload.slyjs = "wait";
      // requirejs.config({
      //   paths: {
      //     "插件名": "插件路径" // 示例: libs/qrcodejs/qrcode
      //   }
      // });
      require(["libs/sly/dist/plugins", "libs/sly/dist/sly.min"], function () {
        window.wwload.slyjs = true;
        replace();
        fncallback();
      });
    } else if (window.wwload.slyjs === "wait") {
      setTimeout(function () {
  ...
  function slyjs1($ele) {
    var config = {};
    config.slyid = $ele.attr("data-slyid");   //设置元素id
    config.startat = $ele.attr("data-startat"); // 设置默认显示选中第几个.从0开始.
    config.speed = $ele.attr("data-speed"); //导航的滑动的速度为300;

    $ele.find('.frame1').attr("id", config.slyid);

    var $frame1 = $('#' + config.slyid);
  ...
  function processElement($ele) {
    ...
    setTimeout(function () {
      $.wwclass.helper.updateProp($ele, "data-x-inited", true);
      $ele.find("li").removeClass("hidden");
    }, 100);
    setTimeout(function () {
      slyjs1($ele);
    }, 300);
